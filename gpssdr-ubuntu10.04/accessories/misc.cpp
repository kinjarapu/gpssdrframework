...
int32_t code_gen(CPX *_dest, int32_t _prn)
{
  ...
  uint32_t G1[1023];
  uint32_t G2[1023];
  uint32_t G1_register[10], G2_register[10];
  uint32_t feedback1, feedback2;
  uint32_t lcv, lcv2;
  uint32_t delay;

	/* G2 Delays as defined in GPS-ISD-200D */
  int32_t delays[51] = {5, 6, 7, 8, 17, 18, 139, 140, 141, 251, 252, 254 ,255, 256, 257, 258, 469, 470, 471, 472,
...
void sine_gen(CPX *_dest, double _f, double _fs, int32_t _samps)
{
  ...
  int16_t c, s;
	...
	{
    c = (int16_t)floor(16383.0*cos(phase));
    s = (int16_t)floor(16383.0*sin(phase));
		_dest[lcv].i = c;
		_dest[lcv].q = s;
 ...
 * */
int32_t run_agc(CPX *_buff, int32_t _samps, int32_t _bits, int32_t _scale)
{
  int32_t lcv, num;
  int16_t max, *p;
  int16_t val;
...
{
  int32_t column_swap[20];  /* Used to keep track of column swapping. */
  int32_t l, k, m;      /* Used for row and column indexing. */
	double pivot;
	double determinate;
...
{
  int32_t angle;    /* The arctangent. */
  int32_t index;        /* Index into the arctangent array. */
  int32_t swap;     /* Used to swap the arguments according to the quadrant. */

  int32_t quadrant;   /* The quadrant of the result (1-4). */
  int32_t sign;     /* The sign of the result. */
