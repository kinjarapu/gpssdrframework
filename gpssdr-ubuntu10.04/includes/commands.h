...
{
  int32_t command_id; //!< Command identifier
  int32_t flag;     //!< Flag for no reason
} Null_C;
...
{
  int32_t command_id; //!< Command identifier
  int32_t chan;     //!< Channel #, or all if chan >= MAX_CHANNELS
} Reset_Channel_C;
...
{
  int32_t command_id; //!< Command identifier
  int32_t sv;     //!< SV #, or all if sv >= NUM_CODES
} Reset_Ephemeris_C;
...
{
  int32_t command_id;   //!< Command identifier
  uint32_t message_id;    //!< Which message id should I set
  uint32_t decimation;    //!< Decimation rate (0 for OFF)
} Set_Periodic_C;
...
typedef struct Set_EEPROM_C
{
  ...
  uint32_t payload[64];   //!< Data to be written
} Set_EEPROM_C;
...
typedef struct Set_Memory_C
{
  ...
  uint32_t checksum;    //!< Checksum of payload
  uint8_t payload[256];   //!< Data to be written
} Set_Memory_C;
...
{
  int32_t command_id; //!< Command identifier
  int32_t sv;     //!< SV #, or all if sv >= NUM_CODES || sv < 0
} Get_Almanac_C;
