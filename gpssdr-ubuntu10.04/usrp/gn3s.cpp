...
gn3s::gn3s(int _which)
{
	...
	fstart = (intptr_t) &_binary_usrp_gn3s_firmware_ihx_start;
	...
	if(fx2_device = usb_fx2_find(GN3S_VID, GN3S_PID, debug, 0)){
		fprintf(stdout, "Found GN3S Device v1\n");
		gn3s_version = 1;
	}
	else if(fx2_device = usb_fx2_find(GN3S_VID_V2, GN3S_PID_V2, debug, 0)){
		fprintf(stdout, "Found GN3S Device v2\n");
		gn3s_version = 2;
	}
	else
	{
		fprintf(stdout, "Found GN3S Device with old VID/PID, reflashing necessary\n");
		/* Program the board */
		ret = prog_gn3s_board();
		if(ret)
		{
