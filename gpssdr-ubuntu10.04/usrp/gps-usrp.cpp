...
// fprintf(stdout," RUN gps-usrp file \n");
...
#define F_L2			(1.57542e9 - 38400)  //(1.2276e9 - 38400)
#define FIFO_SIZE 		(4000) //!< 2 seconds deep?  ************************ why??
//#define GPSPIPE			"/tmp/GPSPIPE"
#define GPSPIPE			"/dev/null"
/*----------------------------------------------------------------------------------------------*/
...
int echo_options(options *_opt)
{
	...
	fprintf(stdout,"echo test in gps-usrp.cpp\n");
	/* Step one, find out what daughterboards are populated */
	/* Open the base RX class */
	usrp_standard_rx_sptr urx = usrp_standard_rx::make(0, 8, 1, -1, 0, 0, 0);
	if(urx == NULL)
	{
	...
	switch(_opt->mode)
	{
        ...
        fprintf(stdout,"GPS-USRP.cpp _opt->mod:	%d\n",_opt->mode); 
        fprintf(stdout,"GPS-USRP.cpp urx->daughterboard_id(0):	%d\n",urx->daughterboard_id(0)); 
        fprintf(stdout,"GPS-USRP.cpp urx->daughterboard_id(1):	%d\n",urx->daughterboard_id(1));
...
int main(int argc, char **argv)
{
	...
	record_options.gr =			40; 	//!< 40 dB of RF gain  //40
	...
	record_options.f_lo_b = 	F_L1;   //F_L2;	//!< Board B L2 by default  //F_L2
	record_options.f_ddc_b = 	0;		//!< no DDC correction
	record_options.bandwidth =	6.0e6 //16.0e6; //!< DBS-RX is set to 10 MHz wide  //16.0e6 dushyanth
...
void *record_thread(void *opt)
{
	...
	/* Make the URX */
	usrp_standard_rx_sptr urx = usrp_standard_rx::make(0, _opt->decimate, 1, -1, 0, 0, 0);
	if(urx == NULL)
	{
		if(_opt->verbose)
	...
	if(urx->daughterboard_id(1) == 2)
	{
		...
		dbs_rx_b->tune(_opt->f_lo_b);
                ...
                fprintf(stdout,"DBS-RX B _opt->mode: \t\t %d\n",_opt->mode);//fuwei
	...
	urx->start();  //return true or false fuwei
...
void wait_for_client()
{
	...
	{
		fifo_pipe = open(GPSPIPE, O_WRONLY | O_NONBLOCK);
		usleep(1000000);
	}
...
void *fifo_thread(void *arg)
{
	...
	remove(GPSPIPE);
	fifo = mkfifo(GPSPIPE, S_IRWXG | S_IRWXU | S_IRWXO);
...
void resample(CPX *_in, CPX *_out, options *_opt)
{
	...
	p_out = (int *)_out;
         ...
         fprintf(stdout,"gps_usrp.cpp opt.f_sample :%15.2f\n",_opt->f_sample); // fuwei
