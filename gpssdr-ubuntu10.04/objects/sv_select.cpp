...
void SV_Select::Import()
{
	...
	uint32_t status, acqs_per_pvt;
	uint32_t bread, k, nsvs;
	...
	if(acqs_per_pvt < 1) acqs_per_pvt = 1;
...
/*----------------------------------------------------------------------------------------------*/
//!< Export state info for the given SV
void SV_Select::Export(int32_t _sv)
{
...
/*----------------------------------------------------------------------------------------------*/
//!< Setup the acq request
uint32_t SV_Select::SetupRequest(int32_t _sv)
{
...
//	}
	...
	if((almanacs[_sv].valid == true) && (mode != ACQ_MODE_COLD))
	{
		...
		command.accel = (int32_t)(ppred->doppler_rate * 4096.0);

		/* Search over the defined range */
		//command.cendopp = (int32_t)ppred->doppler + dt*ppred->doppler_rate;
		command.cendopp = (int32_t)ppred->doppler;
...
/*----------------------------------------------------------------------------------------------*/
//!< Update acq type
/* Does ACQ_MODULO_WEAK weak Acqs every MAX_SV strong ones */
...
/*----------------------------------------------------------------------------------------------*/
//!< Get the most up-to-date almanacs from the ephemeris
void SV_Select::GetAlmanac(int32_t _sv)
{
...
/*----------------------------------------------------------------------------------------------*/
//!< Compute SV positions from almanac
void SV_Select::SV_Position(int32_t _sv)
{
...
/*----------------------------------------------------------------------------------------------*/
//!< Predict states of SVs
void SV_Select::SV_Predict(int32_t _sv)
{
	/* Ugh */
	double rho, theta;
	double fdoppler;
...
/*----------------------------------------------------------------------------------------------*/
//!< Calculate elevation mask angle
...
/*----------------------------------------------------------------------------------------------*/
//!< Get the GEONS data into the proper structure
...
/*----------------------------------------------------------------------------------------------*/
//!< Get the SV prediction
SV_Prediction_M SV_Select::getPrediction(uint32_t _sv)
{
