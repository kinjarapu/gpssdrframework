...
class SV_Select : public Threaded_Object
{
	...
	private:
		...
		int32_t			 mode;						 //!< SV select mode (COLD, WARM, HOT)
		int32_t			 type;						 //!< WEAK or STRONG
		int32_t			 strong_sv;						//!< The current strong SV
		int32_t			 weak_sv;						//!< The current weak SV
		int32_t			 acq_ticks;						//!< Number of acq ticks
	...
	public:
		...
		void Export(int32_t _sv);		 //!< Export state info for the given SV
 		void UpdateState();				//!< Update acq type
 		void Acquire();					//!< Run the acquisition
		void GetAlmanac(int32_t _sv);	 //!< Get the most up-to-date almanacs from the ephemeris
		void SV_Predict(int32_t _sv);	 //!< Predict states of SVs
		void SV_Position(int32_t _sv);	//!< Compute SV positions from almanac
		uint32_t SetupRequest(int32_t _sv); //!< Setup the acq request
		...
		SV_Prediction_M getPrediction(uint32_t _sv); //!< Get the SV prediction
