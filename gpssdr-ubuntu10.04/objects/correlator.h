...
class Correlator : public Threaded_Object
{
	...
	private:
		...
		int32_t				packet_count;						//!< Count 1ms packets
		int32_t				measurement_tic;					//!< Measurement tic
	...
	public:
		...
		void UpdateState(Correlator_State_S *s, int32_t samps);								//!< Update correlator state
		void ProcessFeedback(Correlator_State_S *s, NCO_Command_S *f);						//!< Process the feedback
		void DumpAccum(Correlator_State_S *s, Correlation_S *c, NCO_Command_S *f, int32_t _chan);	//!< Dump accumulation to channel for processing
		void TakeMeasurements();																//!< Take some measurements
		void Accum(Correlator_State_S *s, Correlation_S *c, CPX *data, int32_t samps);		//!< Do the actual accumulation
		void SineGen(int32_t samps);															//!< Dynamic wipeoff generation
};
