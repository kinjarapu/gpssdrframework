...
Telemetry::Telemetry():Threaded_Object("TLMTASK")
{
	...
	pheader = (uint8_t *)&command_header;
	pcommand = (uint8_t *)&command_body;
	pchecksumr = (uint8_t *)&checksumr;
...
/*----------------------------------------------------------------------------------------------*/
void Telemetry::SetType(int32_t _type)
{
...
void Telemetry::ImportSerial()
{
	...
	if(npipe_open)
	{
		bread = read(npipe[READ], &abyte, sizeof(uint8_t));
		while((bread > 0) && npipe_open && grun)
		{
			...
			switch(syncstate)
			{
				...
				case 3: /* Get the checksum */
				{
					...
					if(checksum_bytes >= sizeof(uint32_t))
						StateThree();
...
/*----------------------------------------------------------------------------------------------*/
uint32_t Telemetry::ReadSHU(uint32_t _shu)
{
...
/*----------------------------------------------------------------------------------------------*/
void Telemetry::EmitCCSDSPacket(void *_buff, int32_t _len)
{
	...
	if(npipe_open)
	{
		...
		sbuff = (uint8_t *)&packet_body[0];
		for(lcv = 0; lcv < _len + 18; lcv++)
		{
			write(npipe[WRITE], sbuff, sizeof(uint8_t));
			sbuff++;
		}
	}
...
/*----------------------------------------------------------------------------------------------*/
void Telemetry::setMessageRate(uint32_t _mid, uint32_t _val)
{
...
/*----------------------------------------------------------------------------------------------*/
uint32_t Telemetry::getMessageRate(uint32_t _mid)
{
	if((_mid > FIRST_PERIODIC_M_ID) && (_mid < LAST_PERIODIC_M_ID))
	{
