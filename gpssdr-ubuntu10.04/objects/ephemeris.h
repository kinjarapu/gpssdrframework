...
class Ephemeris : public Threaded_Object
{
	...
	private:
		...
		int32_t 				iode_master[MAX_SV];	//!< IODE flags
	...
	public:
		...
		void Parse(int32_t _sv);					//!< Parse data message into decimal values
		void ParseUTC();						//!< Parse the UTC values
		void ParseHealth();						//!< Parse page 24 of SF 4 and 5 for SV Health codes
		void ParsePage(int32_t _sv_id);			//!< Parse almanac page
		void ClearEphemeris(int32_t _sv);			//!< Dump an ephemeris
		void ClearAlmanac(int32_t _sv);			//!< Dump an almanac
		...
		/* Accessors */
		Ephemeris_M getEphemeris(int32_t _sv);	//!< Get this SV's ephemeris values
		Almanac_M getAlmanac(int32_t _sv);		//!< Get this SV's almanac values
		UTC_Parameter_S getUTC();				//!< Return the utc structure
		Klobuchar_Model_S getKlobuchar();		//!< Return the Klobuchar structure
		int32_t getIODE(int32_t _sv);				//!< Get the current IODE for this SV
		int32_t getEphemerisValidity(int32_t _sv);	//!< Get the decoded status
		int32_t getAlmanacValidity(int32_t _sv);	//!< Get the decoded status
		uint32_t getAlmanacHealth(int32_t _sv);		//!< Get the health status
