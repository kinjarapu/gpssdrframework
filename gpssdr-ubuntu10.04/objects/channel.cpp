...
/*----------------------------------------------------------------------------------------------*/
Channel::Channel(int32_t _chan):Threaded_Object("CHNTASK")
{
	char fname[1024];
...
void Channel::Clear()
{
	...
	I_sum20 = 0;
	Q_sum20 = 0;
  memset(&I_buff, 0x0, 20*sizeof(int32_t));
  memset(&Q_buff, 0x0, 20*sizeof(int32_t));
  memset(&P_buff, 0x0, 20*sizeof(int32_t));
	_20ms_epoch = 0;
	_1ms_epoch = 0;
	best_epoch = 0;

	/* Data message sheit */
  memset(&valid_frame, 0x0, 5*sizeof(int32_t));
	converged = false;
	navigate = false;
	z_lock = false;
	frame_z = 0;
	z_count = 0;
  memset(&word_buff, 0x0, FRAME_SIZE_PLUS_2*sizeof(uint32_t));
...
/*----------------------------------------------------------------------------------------------*/
void Channel::Start(int32_t _sv, Acq_Command_S result, int32_t _corr_len)
{
...
void Channel::FrequencyLock()
{
  ...
  int32_t *p;
	...
	if(count > 1000)
	{
    fft_buff[freq_lock_ticks].i = (int16_t)(it*it - qt*qt);
    fft_buff[freq_lock_ticks].q = (int16_t)(2*it*qt);
		freq_lock_ticks++;
	}
...
{
  int32_t power_buff[20];
  int32_t lcv, new_epoch;
  int32_t thresh_high, thresh_low, bit_lock_err;
...
void Channel::ProcessDataBit()
{
  ...
  uint32_t word0, word1;
	...
	if(frame_lock)
	{
		...
		if(bit_number == 0)
		{
      memcpy(&ephem_packet.word_buff[0], word_buff, FRAME_SIZE_PLUS_2*sizeof(uint32_t));

			if(ValidFrameFormat(&ephem_packet.word_buff[0]))
			{
        sid = (int32_t)((ephem_packet.word_buff[1] >> 8) & 0x00000007);
...
/*----------------------------------------------------------------------------------------------*/
bool Channel::FrameSync(uint32_t word0, uint32_t word1)
{
    uint32_t preamble;           /* The TLM word preamble sequence 10001011. */
    uint32_t sid;                               /* The subframe ID (1 to 5). */
    uint32_t zerobits;         /* The zero bits (the last 2 bits of word 2). */
...
/*----------------------------------------------------------------------------------------------*/
bool Channel::ParityCheck(uint32_t gpsword)
{
...
/*----------------------------------------------------------------------------------------------*/
bool Channel::ValidFrameFormat(uint32_t *subframe)
{
  uint32_t preamble,next_preamble;
  uint32_t zerobits,next_zerobits;
...
void Channel::Export()
{
  ...
  int32_t bwrote;
    ...
    fwrite(&I[0], sizeof(int32_t), 3,  fp);
    fwrite(&Q[0], sizeof(int32_t), 3,  fp);
    fwrite(&P_buff[0], sizeof(int32_t), 20,  fp);
	}
}
/*----------------------------------------------------------------------------------------------*/
