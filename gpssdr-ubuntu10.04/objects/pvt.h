...
class PVT : public Threaded_Object
{
	...
	private:
		...
		int32_t good_channels[MAX_CHANNELS];						//!< Is this a good channel (used to navigate)
		int32_t master_sv[MAX_CHANNELS];							//!< Channel->SV map
		int32_t sv_codes[MAX_CHANNELS];						 //!< Error codes
		int32_t doppler_suspect[MAX_CHANNELS][MAX_CHANNELS];		//!< For the cross-corr check
	...
	public:
		...
		void Navigate();						//!< main navigation task, call the following tabbed functions
			...
			uint32_t PreErrorCheck();			 //!< check all SV's for bad measurements, etc
			void ErrorCheckCrossCorr();			//!< check for cross-correlation problem via ephemeris match
			uint32_t PostErrorCheck();			//!< check all SV's for bad measurements, etc
		...
		void ResetChannel(int32_t _chan);		 //!< Reset individual channel
