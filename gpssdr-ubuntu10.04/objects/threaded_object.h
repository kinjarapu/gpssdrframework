...
class Threaded_Object
{
	...
	protected:
		...
		uint32_t		start_tic;		//!< ISR tic at start of function
		uint32_t		temp_start_tic; //!< ISR tic at start of function (temp)
		uint32_t		stop_tic;	 //!< ISR tic at end of function
		uint32_t		size;		 //!< Size of object
		uint32_t		stack;			//!< Number of bytes on stack
	...
	public:
		...
		uint32_t Trylock();	 //!< Trylock, locks the mutex and returns true, else returns false
		void Stop();			//!< Stop the thread

		uint32_t getExecTic();	//!< Get the execution counter
		uint32_t getStartTic(); //!< Get the 500 us ISR tic at start of function
		uint32_t getStopTic();	//!< Get the 500 us ISR tic at end of function
		uint32_t getSize();	 //!< Get the size of the object
		uint32_t getStack();		//!< Get the stack size
