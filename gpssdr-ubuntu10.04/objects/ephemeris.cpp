...
/*----------------------------------------------------------------------------------------------*/
void Ephemeris::ClearEphemeris(int32_t _sv)
{
	int32_t lcv;
...
/*----------------------------------------------------------------------------------------------*/
void Ephemeris::ClearAlmanac(int32_t _sv)
{
	int32_t lcv;
...
/*----------------------------------------------------------------------------------------------*/
int32_t sbit(uint32_t val, int32_t nbits)
{
	int32_t result = 0x0;
...
void Ephemeris::ParseUTC()
{
	...
	i_tot = (int32_t) TWO_P12 * (int32_t)(utc_data.page[7] >> 14 & 0x000000FF);
	...
	if((i_tot != utc.tot) || (utc.valid == false) || (klobuchar.valid == false))
	{
		...
		utc.tot    	= (int32_t)  TWO_P12 * (int32_t)      (utc_data.page[7] >> 14 & 0x000000FF);
		utc.wnt    	= (uint32_t)                        (utc_data.page[7] >> 6  & 0x000000FF);
		utc.dtls  	= (int32_t)                    (sbit(utc_data.page[8] >> 22,  8));
		utc.wnlsf 	= (uint32_t)                        (utc_data.page[8] >> 14 & 0x000000FF);
		utc.dn     	= (uint32_t)                        (utc_data.page[8] >> 6  & 0x000000FF);
		utc.dtlsf 	= (int32_t)                    (sbit(utc_data.page[9] >> 22,  8));
		utc.valid = true;
	}
...
/*----------------------------------------------------------------------------------------------*/
void Ephemeris::Parse(int32_t _sv)
{
	int32_t IODE[3];
	...
	if((_sv < MAX_SV) && (_sv >= 0))
	{
		if((IODE[0] == IODE[1]) && (IODE[1] == IODE[2]) && (IODE[0] == IODE[2]))
		{
			...
			ephemerides[_sv].iodc =						(int32_t)		((ephem_data[_sv].subframe_1[2] << 2   & 0x00000300) | (ephem_data[_sv].subframe_1[7] >> 22 & 0x000000FF));
			...
			ephemerides[_sv].ura =						(int32_t)		((ephem_data[_sv].subframe_1[2] >> 14) & 0x0000000F);
			ephemerides[_sv].iode =						(int32_t)		((ephem_data[_sv].subframe_1[7] >> 22) & 0x000000FF);
			ephemerides[_sv].tow =		4*				(int32_t)		((ephem_data[_sv].subframe_1[1] >> 13) & 0x0000FFFF);
			ephemerides[_sv].week_number =				(int32_t)		((ephem_data[_sv].subframe_1[2] >> 20) & 0x000003FF);
			ephemerides[_sv].subframe_1_health =		(int32_t)		((ephem_data[_sv].subframe_1[2] >> 8)  & 0x0000003F);
			ephemerides[_sv].a = ephemerides[_sv].sqrta * ephemerides[_sv].sqrta;
			ephemerides[_sv].tofxmission = (int32_t) (ephemerides[_sv].tow * 1.5);
...
/*----------------------------------------------------------------------------------------------*/
void Ephemeris::ParsePage(int32_t _sv_id)
{
...
/*----------------------------------------------------------------------------------------------*/
Ephemeris_M Ephemeris::getEphemeris(int32_t _sv)
{
...
/*----------------------------------------------------------------------------------------------*/
Almanac_M Ephemeris::getAlmanac(int32_t _sv)
{
	return(almanacs[_sv]);
}
/*----------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------*/
int32_t Ephemeris::getIODE(int32_t _sv)
{
...
/*----------------------------------------------------------------------------------------------*/
int32_t Ephemeris::getEphemerisValidity(int32_t _sv)
{
...
/*----------------------------------------------------------------------------------------------*/
int32_t Ephemeris::getAlmanacValidity(int32_t _sv)
{
...
/*----------------------------------------------------------------------------------------------*/
uint32_t Ephemeris::getAlmanacHealth(int32_t _sv)
{
