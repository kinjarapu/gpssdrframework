...
class FFT
{
	...
	private:
		...
		int32_t *BRX;					//!< Re-order temp array
		int32_t *BR;					//!< Re-order index array

		int32_t N;					//!< Length (should be 2^N!!!)
		int32_t M;					//!< Log2(N) (number of ranks)
		int32_t R[16];				//!< Programmable rank scaling
	...
	public:
		...
		FFT();								//!< Initialize FFT
		FFT(int32_t _N);						//!< Initialize FFT for 2^N
		FFT(int32_t _N, int32_t _R[MAX_RANKS]);			//!< Initialize FFT for 2^N, with ranks
		~FFT();								//!< Destructor
