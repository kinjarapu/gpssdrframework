...
class Telemetry : public Threaded_Object
{
	...
	private:
		...
		uint32_t packet_tic;							//!< Count the number of messages
		uint32_t command_tic;						 //!< Count the number of commands
		uint32_t export_messages;					 //!< New data from PVT is here, now transmit
		uint32_t export_ekf;							//!< New data from the EKF is here, now transmit
		uint32_t export_tic;							//!< Count number of exports
		uint32_t last_start_tic;						//!< Special variable to allow the telemetry to time itself
		uint32_t last_stop_tic;					 //!< Special variable to allow the telemetry to time itself
		uint32_t missed_interrupts;				 //!< Keep track of missed interrupts
		uint32_t adc_values[SHU_SIGNALS];			 //!< This must be kept track of
		uint32_t msg_rates[LAST_M_ID+1];				//!< Control output of periodic messages, 0 == off, !0 == decimation rate
		void (Telemetry::*msg_handlers[LAST_M_ID+1])(void);	//!< Function pointers for periodic message handlers

		int32_t tlm_type;
		int32_t fifo[2];
		int32_t npipe[2];							 //!< Named pipe and/or serial interface
		int32_t npipe_open;						 //!< Is the named pipe open!?
		...
		uint32_t syncstate;						 //!< Flag to indicate synchronization step 0 for nothing, 1 for preamble, 2 for CCSDS header, 3 for packet
		uint32_t syncword;							//!< Synchronize to the 0xAAAAAAAA preamble
		uint32_t checksumr;						 //!< Received checksum

		uint8_t *pchecksumr;							//!< Pointer to received checksum
		uint32_t checksum_bytes;						//!< Bytes to read in checksum

		uint8_t *pheader;							 //!< Pointer to header
		uint32_t header_bytes;						//!< Bytes left to read in header

		uint8_t *pcommand;							//!< Pointer to command
		uint32_t command_bytes;					 //!< Bytes read in command
		uint32_t command_bytes_2_read;				//!< Total bytes in body of command

		uint32_t bytes_read;							//!< Count number of received bytes
		uint32_t bytes_sent;							//!< Count number of transmitted bytes
	...
	public:
		...
		~Telemetry();
		void SetType(int32_t _type);					//!< Serial or named pipe
		void OpenPipe();
		void OpenSerial();
		void ClosePipe();
		...
		uint32_t ReadSHU(uint32_t _shu);				//!< Read the status and health A/D

		/* Object specific methods */
		//void setSerial(NU_SERIAL_PORT *_sport){sport = _sport;};	//!< Grab the serial pointer
		void EmitCCSDSPacket(void *_buff, int32_t _len);				//!< Emit a CCSDS packet
		...
		void setMessageRate(uint32_t _mid, uint32_t _val);	//!< Set the message rate
		uint32_t getMessageRate(uint32_t _mid);			 //!< Get the message rate
