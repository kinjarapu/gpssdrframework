...
void Correlator::TakeMeasurements()
{
	...
	double tcode, tphase;
	int32_t lcv;
	uint32_t index_dp;//!< double previous
	uint32_t index_p;	//!< previous
	uint32_t index_c;	//!< current
	uint32_t nav_dp, nav_p, nav_c;
	Correlator_State_S *s;
	...
	for(lcv = 0; lcv < MAX_CHANNELS; lcv++)
	{
		...
		if(s->navigate)
		{
			...
			aMeasurement->carrier_rate		 = (uint32_t)floor(s->carrier_nco * HZ_2_NCO_CARR_INCR + 0.5);

			tcode = floor(s->code_phase_mod);
			aMeasurement->code_phase         = (uint32_t)tcode;
			aMeasurement->frac_code_phase    = (uint32_t)((s->code_phase_mod - tcode) * TWO_P31);

			tphase = floor(s->carrier_phase);
			aMeasurement->carrier_phase      = (uint32_t)tphase;
			aMeasurement->frac_carrier_phase = (uint32_t)(s->carrier_phase_mod * TWO_P32);
...
/*----------------------------------------------------------------------------------------------*/
void Correlator::UpdateState(Correlator_State_S *s, int32_t samps)
{
...
/*----------------------------------------------------------------------------------------------*/
void Correlator::Accum(Correlator_State_S *s, Correlation_S *c, CPX *data, int32_t samps)
{
	...
	c->I[0] += (int32_t) EPL[0].i;
	c->I[1] += (int32_t) EPL[1].i;
	c->I[2] += (int32_t) EPL[2].i;

	c->Q[0] += (int32_t) EPL[0].q;
	c->Q[1] += (int32_t) EPL[1].q;
	c->Q[2] += (int32_t) EPL[2].q;
...
void Correlator::InitCorrelator(Correlator_State_S *s)
{
	...
	s->_1ms_epoch 			= 0;
	s->_20ms_epoch			= 0;
	s->rollover 			= (int32_t) ceil(((double)CODE_CHIPS - code_phase)*SAMPLE_FREQUENCY/s->code_nco); /* Calculate rollover point */
	...
	bin = (int32_t) floor((code_phase - 0.5)*CODE_BINS + 0.5) + CODE_BINS/2;
	if(bin < 0)	bin = 0; if(bin > 2*CODE_BINS) bin = 2*CODE_BINS;
	s->pcode[2] = s->code_rows[bin];
	s->pcode[2] += inc;
	s->cbin[2] = bin;
