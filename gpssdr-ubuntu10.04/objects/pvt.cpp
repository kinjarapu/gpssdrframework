...
//	integer_second = (int32_t)floor(master_clock.time + .5);
...
void PVT::UTCTime()
{
	...
	if(utc.valid)
	{
		...
		tot.day = (uint32_t)floor((master_clock.time - dtutc)/SECONDS_IN_DAY);
...
void PVT::PseudoRange()
{
	...
	for(lcv = 0; lcv < MAX_CHANNELS; lcv++)
	{
		if(good_channels[lcv])
		{
			...
			time_rate = (cp_prev - cp)*cp_adjust ; //!< Note the sign flip
...
/*----------------------------------------------------------------------------------------------*/
uint32_t PVT::PreErrorCheck()
{
	int32_t lcv;
	int32_t num_edited_channels = 0;
	int32_t nav_channels;
	double radius;
...
/*----------------------------------------------------------------------------------------------*/
void PVT::ResetChannel(int32_t _chan)
{
