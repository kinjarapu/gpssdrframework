...
class Commando : public Threaded_Object
{
	...
	private:
		...
		uint32_t command_tic;							//!< Count the number of executed commands
	...
	public:
		...
		void PendEEPROM(uint32_t _bank);				//!< Wait for the EEPROM's ready bit to go high
		void PageWrite(uint32_t _bank, uint32_t *_addr, uint32_t *_data, uint32_t _words); //!< Write a page in the EEPROM
		...
		void EmitCCSDSPacket(void *_buff, uint32_t _len);	//!< Dump packet header and body to Telemtry via pipe
