...
static uint32_t SIZEOF_M[LAST_M_ID + 1] =
{
  0,
  0,
...
/*----------------------------------------------------------------------------------------------*/
void GUI_Serial::setIO(int32_t _serial)
{
...
/*----------------------------------------------------------------------------------------------*/
void endian_swap(void *_b, int32_t _bytes, int32_t _flag)
{

  uint8_t *pt8, *pb8;
  uint32_t *p32;
  uint32_t temp;
  int32_t dwords, rem, lcv;
...
/*----------------------------------------------------------------------------------------------*/
int GUI_Serial::Read(void *_b, int32_t _bytes)
{

  int32_t nbytes, bread, k;
  uint8_t *buff;
...
/*----------------------------------------------------------------------------------------------*/
int GUI_Serial::Write(void *_b, int32_t _bytes)
{
...
{
  uint32_t checksumc, checksumr;
  int32_t nbytes, bread, k, chan;
  uint8_t v;
  ...
  if(message_sync)
  {
    ...
    byte_count += Read(&syncword, sizeof(uint32_t));
    ...
    byte_count += Read(&checksumr, sizeof(uint32_t));
    ...
    byte_count += Read(&postword, sizeof(uint32_t));
  ...
  else
  {
    ...
    if(syncword == 0xAAAAAAAA)
    {
      ...
      if(CheckPacket(&decoded_packet) == true)
      {
        ...
        Read(&checksumr, sizeof(uint32_t));
        Read(&postword, sizeof(uint32_t));
        if(postword != 0xBBBBBBBB)
        {
          message_sync = 0;
          packet_count[LAST_M_ID]++;
...
/*----------------------------------------------------------------------------------------------*/
void GUI_Serial::FixDoubles(void *_b, int32_t _num)
{
...
void GUI_Serial::writeGPS()
{
  ...
  uint8_t *sbuff;
  uint32_t pre = 0xAAAAAAAA;
  uint32_t post = 0xBBBBBBBB;
  uint32_t checksum;
...
/*----------------------------------------------------------------------------------------------*/
void GUI_Serial::formCommand(int32_t _id, void *_p)
{

  uint32_t pend, cmd_count;
  ...
  switch(_id)
  {
    ...
    case RESET_CHANNEL_C_ID:
      cb->reset_channel.chan = *(int32_t *)_p;
    ...
    case GET_EPHEMERIS_C_ID:
      cb->get_ephemeris.sv = *(int32_t *)_p;
    ...
    case GET_SV_PREDICTION_C_ID:
      cb->get_sv_prediction.sv = *(int32_t *)_p;
...
/*----------------------------------------------------------------------------------------------*/
int32_t GUI_Serial::peekCommand()
{
...
/*----------------------------------------------------------------------------------------------*/
void GUI_Serial::printPseudo(int32_t _chan)
{
...
/*----------------------------------------------------------------------------------------------*/
void GUI_Serial::printMeas(int32_t _chan)
{
...
/*----------------------------------------------------------------------------------------------*/
void GUI_Serial::printChan(int32_t _chan)
{
  SPS_M   *pNav = &messages.sps;
...
/*----------------------------------------------------------------------------------------------*/
void GUI_Serial::printSVPos(int32_t _chan)
{
...
/*----------------------------------------------------------------------------------------------*/
void GUI_Serial::printSVPred(int32_t _sv)
{
  SPS_M *pNav = &messages.sps;
